...
class ResponseOrPlan(BaseModel):
    ...
    response: str = Field(description="如果问题已解决，请给出对用户的最终回答。如果未解决，留空。")
    plan: List[str] = Field(description="如果未解决，给出的剩余步骤。")
...
async def planner_node(state: PlanExecuteState) -> dict:
    ...
    planner_prompt = ChatPromptTemplate.from_template(
        ...
        "要求：步骤颗粒度要小，不要包含过于抽象的词汇。只需返回步骤清单。\n"
        "目标：{objective}"
    )
    
    # 强制模型输出符合 Plan(Pydantic) 结构的 JSON
    planner_chain = planner_prompt | llm.with_structured_output(Plan)
    plan = await planner_chain.ainvoke({"objective": state["input"]})
...
async def executor_node(state: PlanExecuteState) -> dict:
    ...
    # 在真实的生产环境中，这里会调用 tools_node
    # 为了演示 Plan 架构的纯粹性，这里我们让 LLM 充当执行器去分析并给出执行结果
...
async def replanner_node(state: PlanExecuteState) -> dict:
    ...
    replanner_prompt = ChatPromptTemplate.from_template(
        ...
        "如果可以，请在 response 字段中输出对用户的最终答复，并清空 plan。\n"
        "如果还不够，或者遇到了死胡同，请在 plan 字段中重新规划剩余的步骤。"
    )
    
    past_steps_str = "\n".join([f"步骤: {s}\n结果: {r}" for s, r in state.get("past_steps", [])])
    
    chain = replanner_prompt | llm.with_structured_output(ResponseOrPlan)
    output = await chain.ainvoke({"objective": state["input"], "past_steps": past_steps_str})
