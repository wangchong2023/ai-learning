...
路径：`source/rag_app/main.py` -> `source/utils/factory.py`
...
系统判断是否需要外部信息：
*   如果问题涉及“天气/实时”，则调用 `source/tools/common_tools.py`。

---

## 🛡️ 2. LangGraph：受控的智能体 (Walkthrough)

路径：`source/langgraph_app/core/`
...
路径：`source/multi_agent_app/core/`
...
*   **想增加新工具？** 只需要修改 `source/tools/common_tools.py` 并注册到字典中。
*   **想更换模型？** 修改 `env/.env` 中的 API Key 和 `source/utils/config.py` 中的模型名称。
*   **想增加测试？** 在 `source/tests/` 下添加新的 `unittest` 类并运行 `make test`。
