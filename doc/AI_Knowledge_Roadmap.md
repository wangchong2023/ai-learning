...
> + **L1 使用者**：通过 [9. 编码助手](#9-编码助手) 实现即时研发提效。
> + **L2 & L3 开发者**：从 [10. 通用框架](#10-通用框架) 进阶至 [5.4 L5.4 : 开发者](#54-l3-开发者) 与 [21. RAG 落地与性能调优](#21-rag-落地与性能调优)。
> + **L4 架构师**：务必参考 [15. 质量评估与可观测性](#15-质量评估与可观测性)、[26. 治理、安全与合规](#26-治理安全与合规) 及 [23. 模型微调与强化实战](#23-模型微调与强化实战)。
>
...
| :---: | :---: | :---: | :---: | :---: |
| 数学基础 | 线性代数 | [向量](#71111-向量)、[矩阵](#71112-矩阵)乘法、特征值分解，是神经网络运算底层语言 | 神经网络权重计算 | [3Blue1Brown 线性代数](https://www.3blue1brown.com/topics/linear-algebra) |
| 数学基础 | 微积分与反向传播 | 链式法则驱动[梯度下降](#7116-梯度下降)，是深度学习训练的核心机制 | 模型训练、损失优化 | [Calculus - MIT OCW](https://ocw.mit.edu/courses/18-01sc-single-variable-calculus-fall-2010/) |
...
### 7.1 原理基座
> [!NOTE]  
> **小白逃生通道**：本节涉及较多底层数学原理。如果你并非算法工程师，或更关注“如何使用和部署 AI”，**可以安全地跳过本节**，直接从 [7.8 交互增强](#78-交互增强) 开始阅读。这不会影响你对后续工程架构的理解。
...
> [!TIP]  
**深挖提示**：关于 SFT 样本的标准化格式、LoRA 微调的 Python 实现以及最新的 **GRPO 强化训练** 细节，请跳转至 [23. 模型微调与强化实战](#23-模型微调与强化实战)。
>
...
Agent 的核心能力在于跨越单次推理的限制。通过持久化状态实现长效认知，详见 [19.2 ](#192-记忆系统)。
...
| **编写第一行 Agent 代码** | [19. 单体智能体](#19-单体智能体) | 掌握 Function Calling 与 Agent Loop 闭环 | **Build** |
| **训练私有模型** | [23. 模型微调与强化实战](#23-模型微调与强化实战) | 通过 SFT 与 GRPO 强化实现领域能力闭环 | **Train** |
| **K8s 容器化部署** | [24. 私有化与 K24. s 容器化部署](#24-私有化与-k8s-容器化部署) | 实现企业级算力调度与模型高可用部署 | **Deploy** |
| **全链路监控运维** | [25. 基础设施与全链路监控运维](#25-基础设施与全链路监控运维) | 建立 AgentOps 监控与 EDD 自动化评估体系 | **Operate** |
| **建立全链路安全风控** | [26. 治理、安全与合规](#26-治理安全与合规) | 构建提示词注入拦截与行为审计护栏 | **Operate** |
| **降本增效优化** | [26.3 管理与工程效能](#263-管理与工程效能) | 利用语义缓存与流式响应降低成本提速 | **Operate** |
| **沉淀架构心法** | [26.3.3 架构心法与风险管理](#2633-架构心法与风险管理) | 确立"确定性优先"的高阶架构哲学 | **Strategy** |
...
### 19.3 智能体设计模式
> **理论前置**：在动手编写代码前，请确保您已深度理解 [7.9.4 设计模式](#794-设计模式) 中的核心差异。
> 
...
## 20. 多智能体实战
> **理论前置**：关于集中式、对话式与角色驱动的深度架构分析，请参考 [7.9.5 多智能体协作](#795-多智能体协作)。
> 
...
### 21.1 文本 RAG
**工程逻辑**：遵循原理篇中的“全生命周期闭环”，在代码实现上通过工具链集成实现自动化。详细原理图请参考 [7.8.4 检索增强生成](#784-检索增强生成)。
