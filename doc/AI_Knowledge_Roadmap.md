...
| **Layer 3: 编排** | **架构与流程控制** | LangGraph, MCP | **骨架 (Skeleton)** | 将不确定的输出转化为确定的逻辑流。 |
...
timeline
    ...
    2017-2024   : 涌现时代 (3.0 - System 1) : 逻辑从【海量参数】中涌现; 核心: 直觉概率, 存在幻觉。
...
*   **RNN (序列依赖瓶颈)**：信息呈 **“漏斗式”** 流动。随着序列增长，前期特征因梯度消失而产生不可逆衰减。
```mermaid
graph LR
...
| :--- | :--- | :--- | :--- | :--- |
| **NVIDIA** | 计算霸主 | H200, B200, RTX 5090 | 极致的 CUDA 生态与 HBM 带宽 | 训练与大规模生产推理的首选 |
| **Apple** | 端侧王者 | M3/M4 Max/Ultra | **统一内存架构 (UMA)** | 70B 以下模型本地开发最具性价比 |
| **AMD** | 性价比方案 | MI300X/MI325X | 显存带宽领先，生态追赶中 | 预算敏感型的大规模分布式推理 |
| **Huawei** | 自主领军 | Ascend 910B/C | 国产算力天花板，CANN 生态 | 国内合规训练与推理的主力方案 |
